...
import { system, CustomCommandParamType, CommandPermissionLevel, CustomCommandStatus, Player, Entity, world, CustomCommandSource, } from "@minecraft/server";
...
export class CMD {
    ...
    addBoolean(name, require = true) {
        if (require && this.#commandObj.optionalParameters.length > 0)
            throw new Error("can't add required parameters after optional parameters");
        const arr = require
     ...
     * @throws if its a required parameters, but added after optional parameters
     */
    addEnum(name, require = true, value = []) {
        if (require && this.#commandObj.optionalParameters.length > 0)
            throw new Error("can't add required parameters after optional parameters");
        const arr = require
            ? this.#commandObj.mandatoryParameters
        ...
        if (!ccEnum.find((v) => v.name === enumName))
            RegisterEnum(enumName, value);
    ...
    run = (source, ...args) => {
        ...
        // check tag on entity
        const entity = source.sourceType === CustomCommandSource.Entity
            ? source.sourceEntity
            : source.initiator;
        ...
        system.run(async () => {
            ...
            if ([CustomCommandSource.Server, CustomCommandSource.Block].includes(source.sourceType))
                world
                    .getPlayers()
                    .filter((p) => p.commandPermissionLevel >= this.#commandObj.permissionLevel)
    ...
    register() {
        if (!this.#func)
            throw new Error("command need function to run");
        if (this.#commandObj.name === "")
            throw new Error("command need a name");
...
export function helpCmd() {
    new CMD()
        ...
        .setFunction((event) => {
        if (event.source.sourceType !== CustomCommandSource.Entity ||
        ...
        if (argument && Is.String(argument)) {
            if (/^\d+$/.test(argument.trim()))
                page = parseInt(argument.trim());
            else {
                const foundIt = ccList.find((cmdObj) => cmdObj.getName() === argument ||
                ...
                player.sendMessage(`§e${cmd.name} (or ${getNameOnly(cmd.name)}${alias.length > 0 ? "|" + alias.join("|") : ""} ):`);
        ...
        player.sendMessage(`---- ${CONFIG.prefix} ${CONFIG.helpCommand || "helpCmd"} page ${page} of ${Math.ceil(ccList.length / 10)} ----`);
